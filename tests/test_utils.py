...
from order_package.utils import level, level_cost, optimal, display_level, display_optimal
...
class test_utils(unittest.TestCase):
    ...
    def test_optimal(self):
        ...
        self.assertEqual(optimal(desired)[1], [0,[6,1],[5,2],[4,3]])
        
        # Returns the correct the order of enchantments for odd size
        self.assertEqual(optimal(desired[:-1])[1], [0,[5,1],[4,2],3])
        self.assertEqual(optimal(desired[:-3])[1], [0,[3,1],2])
        ...
        # Handles no pairs
        no_pairs = [12]
        self.assertEqual(optimal(no_pairs), (no_pairs[0], [0]))
